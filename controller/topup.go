...
func GetTopUpInfo(c *gin.Context) {
	...
	enableAlipay := complianceConfirmed && isAlipayEnabled()
	...
	if enableWaffo {
		if !containsPayMethodType(payMethods, model.PaymentMethodWaffo) {
	...
	if enableWaffoPancake {
		if !containsPayMethodType(payMethods, model.PaymentMethodWaffoPancake) {
	...
	data := gin.H{
		...
		"payment_notice":          operation_setting.GetPaymentSetting().PaymentNotice,
