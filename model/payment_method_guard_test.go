...
func TestEnterpriseAlipaySettlementPreservesCNYAndWalletLimits(t *testing.T) {
	...
	t.Cleanup(func() { common.QuotaPerUnit, operation_setting.USDExchangeRate = oldUnit, oldRate })
	for _, tc := range []struct {
		...
		{"CNY conversion", "enterprise_alipay_cny", 14, 5, 1000005, false},
		{"USD conversion", "enterprise_alipay", 2, 5, 1000005, false},
		{"wallet ceiling", "enterprise_alipay_cny", 14, common.MaxWalletQuota, common.MaxWalletQuota, true},
		{"oversized order", "enterprise_alipay", 9223372036854775807, 5, 5, true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			truncateTables(t)
			user := insertUserForPaymentGuardTest(t, 601, tc.balance)
			order := &TopUp{UserId: user.Id, Amount: tc.amount, Money: 14, TradeNo: "ALIPAYMERGE", PaymentProvider: PaymentProviderAlipay, PaymentMethod: tc.method, Status: common.TopUpStatusPending}
			require.NoError(t, order.Insert())
			err := RechargeAlipay(order.TradeNo)
			if tc.fail {
				require.Error(t, err)
				assert.Equal(t, common.TopUpStatusPending, getTopUpStatusForPaymentGuardTest(t, order.TradeNo))
			} else {
				require.NoError(t, err)
				require.NoError(t, RechargeAlipay(order.TradeNo))
				assert.Equal(t, common.TopUpStatusSuccess, getTopUpStatusForPaymentGuardTest(t, order.TradeNo))
			}
			assert.Equal(t, tc.want, getUserQuotaForPaymentGuardTest(t, user.Id))
		})
	}
}
