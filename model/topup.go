...
const (
	...
	PaymentProviderAlipay       = "enterprise_alipay"
...
func ManualCompleteTopUp(tradeNo string, callerIp string) error {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if topUp.PaymentProvider == PaymentProviderStripe {
			...
			quotaToAdd, quotaErr = common.WalletQuotaFromDecimalStrict(decimal.NewFromInt(topUp.Amount).Div(decimal.NewFromFloat(operation_setting.USDExchangeRate)).Mul(decimal.NewFromFloat(common.QuotaPerUnit)))
		} else {
			quotaToAdd, quotaErr = common.WalletQuotaFromDecimalStrict(
				decimal.NewFromInt(topUp.Amount).Mul(decimal.NewFromFloat(common.QuotaPerUnit)),
			)
...
func RechargeAlipay(tradeNo string) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		case "enterprise_alipay_cny":
			...
			quotaToAdd, quotaErr = common.WalletQuotaFromDecimalStrict(dAmount.Div(dRate).Mul(dQuotaPerUnit))
	...
	if quotaToAdd > 0 {
		RecordLog(topUp.UserId, LogTypeTopup, fmt.Sprintf("企业支付宝充值成功，充值额度: %v，支付金额: %.2f", logger.FormatQuota(quotaToAdd), topUp.Money))
	}
