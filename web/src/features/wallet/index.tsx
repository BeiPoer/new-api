...
function decodeBasicHtmlEntities(value: string) {
  return value
    ...
    .replaceAll(/&(amp|lt|gt|quot|apos|#39);/g, (entity) => {
...
function renderPaymentNoticeContent(paymentNotice: string): ReactNode[] {
  ...
  paymentNotice.replaceAll(
    ...
    (match, _quote: string, href: string, label: string, offset: number) => {
      ...
      if (safeHref) {
        const text =
          decodeBasicHtmlEntities(label.replaceAll(/<[^>]*>/g, '')).trim() ||
          safeHref
        nodes.push(
          <a
...
export function Wallet(props: WalletProps) {
  ...
  }, [topupInfo, calculatePaymentAmount, shouldUseAlipayDirectCNY])
      ...
      selectedWaffoMethodIndex,
      {
        regular: (amount, paymentType) =>
          processPayment(
            amount,
            paymentType,
            paymentType === PAYMENT_TYPES.ENTERPRISE_ALIPAY &&
          ...
          <div className='mx-auto flex w-full max-w-7xl flex-col gap-4 sm:gap-5'>
            ...
            {shouldShowPaymentNotice ? (
              <Alert className='border-yellow-200 bg-yellow-50 text-yellow-950 dark:border-yellow-900/60 dark:bg-yellow-950/30 dark:text-yellow-100'>
                <AlertDescription className='break-words whitespace-pre-wrap text-yellow-900 dark:text-yellow-100'>
